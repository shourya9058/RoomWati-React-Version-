...
/* All listings page*/ 
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('[data-initial-favorite-count]').forEach(el => {
      const count = Math.max(0, parseInt(el.getAttribute('data-initial-favorite-count'), 10));
      if (!isNaN(count)) {
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (data.status === 'added') {
                ...
                const favoritesFilter = document.querySelector('.filter-btn:nth-child(2)');
                if (favoritesFilter && favoritesFilter.classList.contains('active')) {
                  ...
                  if (listingCard) {
                    ...
                    setTimeout(() => {
                      ...
                      const remainingFavorites = document.querySelectorAll('.listing-col').length;
                ...
                if (window.location.pathname.includes('favorites') || window.location.pathname.includes('profile')) {
                  const listingCard = this.closest('.user-listing-card, .listing-card, .card');
                  if (listingCard) {
                    ...
                    setTimeout(() => {
                      ...
                      const favoritesContainer = document.querySelector('.favorites-container, #favorites-tab, .user-listings-grid');
                ...
                if (window.location.pathname === '/') {
                  ...
                  if (activeFilterBtn && activeFilterBtn.querySelector('.fa-heart')) {
                    const listingCard = this.closest('.listing-card, .card, .user-listing-card, .listing-col');
                    if (listingCard) {
                      ...
                      setTimeout(() => {
                        ...
                        if (window.listings) {
                          window.listings = window.listings.filter(listing => {
                            return listing.getAttribute('data-listing-id') !== listingId;
                          });
                        ...
                        const favoritesGrid = document.querySelector('.listings-grid');
                        const visibleFavorites = Array.from(document.querySelectorAll('.listing-col[data-is-favorite="true"]'));
                        
                        if (visibleFavorites.length === 0 && favoritesGrid) {
                          const emptyState = document.createElement('div');
                          emptyState.className = 'col-12 text-center my-5 no-favorites-message';
                          emptyState.innerHTML = `
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (successMessage && successMessage.textContent.includes('Profile updated')) {
